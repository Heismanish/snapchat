...
type ChatMessageProps = {
  messages: IMessageDocument[] | PopulatedDoc<IUserDocument>[];
  session: Session;
};
...
const ChatMessages = ({ messages, session }: ChatMessageProps) => {
  ...
  return (
    <>
      ...
      <Dialog
        open={isPreviewingImage.opened}
        onOpenChange={() =>
          setIsPreviewingImage({ opened: false, imageUrl: "" })
        }
      >
        <DialogContent
          className="max-w-4xl h-3/4 bg-sigMain border border-sigBgBorder outline-none"
          autoFocus={false}
        >
          <Image
            src={isPreviewingImage.imageUrl}
            fill
